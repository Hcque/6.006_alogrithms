...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        while len(Q) > 0:
            u = Q.extract_min()
            # print(len(Q))
            node, dist = u.node, u.distance
            count = count + 1
            if node is destination:
                break
            for next_node in node.adj: # relaxation
                next_dist = dist + weight(node, next_node)
                if next_node.queue_key is None:
                    next_node.queue_key = NodeDistancePair(next_node, next_dist)
                    Q.insert(next_node.queue_key)
                    next_node.parent = node
                elif next_node.queue_key.distance > next_dist:
                    next_node.queue_key.distance = next_dist
                    Q.decrease_key(next_node.queue_key)
                    next_node.parent = node
